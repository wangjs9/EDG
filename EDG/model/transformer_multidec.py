...
class Transformer_multidec(nn.Module):
    ...
    def train_one_batch(self, batch, iter, train=True):
        ...
        # print("===============================================================================")
        if (config.oracle): attention_parameters = self.attention_activation(
        ...
        if config.softmax:
            loss = self.criterion(logit.contiguous().view(-1, logit.size(-1)),
                                  dec_batch.contiguous().view(-1)) + nn.CrossEntropyLoss()(logit_prob, torch.LongTensor(
