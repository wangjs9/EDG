...
import numpy as np
import math
from model.common_layer import EncoderLayer, DecoderLayer, LayerNorm , \
...
class Latent(nn.Module):
    def __init__(self, hidden_dim, filter, device, dropout=0):
    ...
    def forward(self, x, x_p):
        mean = self.mean(x)
        log_var = self.var(x)
        eps = torch.randn(x.size()).to(self.device)
...
class Encoder(nn.Module):
    ...
    def forward(self, inputs, mask, cazprob=None):
        # Add input dropout
        ...
        # if cazprob != None:
        #     cazprob = 2 * (cazprob.unsqueeze(-1).repeat(1, 1, x.size(-1)) - 0.5)
        # else:
        #     cazprob = torch.zeros_like(x)
        if self.universal:
            if config.act:
...
class Transformer_CVAE(nn.Module):
    def __init__(self, vocab, decoder_number, model_file_path=None, is_eval=False, load_optim=False):
        ...
        self.cazpos_embeding = nn.Embedding.from_pretrained(posembedding, freeze=True)
        ...
        self.latent_layer = Latent(config.hidden_dim, config.filter, self.device)
    ...
    def train_one_batch(self, batch, iter, train=True):
        ...
        cazpos = self.cazpos_embeding(enc_causepos)
        encoder_outputs = self.encoder(self.embedding(enc_batch) + emb_mak + cazpos,
        ...
        ## Caz VAE
        kld_loss, z = self.latent_layer(encoder_outputs[:, 0], caz_encoder_outputs[:, 0])
        ...
        loss = self.criterion(logit.contiguous().view(-1, logit.size(-1)), dec_batch.contiguous().view(-1)) + \
               config.kl_ceiling * kl_weight * kld_loss
...
class Transformer_ECE(nn.Module):
    def __init__(self, vocab, decoder_number, model_file_path=None, is_eval=False, load_optim=False):
        """
        vocab: a Lang type data, which is defined in data_reader.py
        decoder_number: the number of classes
        """
        super(Transformer_ECE, self).__init__()
        self.device = torch.device('cuda' if config.USE_CUDA else 'cpu')
        self.vocab = vocab
        self.vocab_size = vocab.n_words
